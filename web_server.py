...
def add_car_position_to_file(filepath: str, position_number: int, car_image_name: str, 
                             owner_username: str, initial_velocity: int = 0) -> bool:
    """
    Locates the matching StartingPosition marker and appends a structured CarPosition object record,
    injecting the custom driver velocity value selected at scenario entry.
    """
    ...
    if starting is None:
        print(f'No StartingPosition asset marker found for configuration matching index: {position_number}')
        return False
    ...
    car_entry = {
        ...
        # ── VELOCITY INITIALIZATION ENGINE VARIABLES ──
        'current_speed': initial_velocity, # ← Dynamically set to the driver's custom slider choice!
        'top_speed': 120,                  # Standard baseline speed metric fallback envelope
...
def create_game():
    ...
    initial_speed = data.get('initial_speed', 0)
...
def join_game():
    ...
    try:
        ...
        car_image_name = data.get('car_image_name', '')
        t1p0_path = find_t1p0_path(game_id, game['name'])
        ...
        if game_now_full and game['auto_start']:
            db.update_game_status(game_id, 'active')
            return jsonify({
                'message': f'{session["username"]} joined — game is full and auto-started!',
                'auto_started': True,
                'game_file': os.path.basename(t1p0_path) if t1p0_path else ''
            }), 200

        return jsonify({
            'message': f'{session["username"]} joined the game',
            'auto_started': False
        }), 200
    except Exception as e:
...
def handle_game_maneuver():
    ...
    Runs validation rules, runs engine vector math, stages a ProposedCarPosition 
    ghost asset string, and synchronizes the active resource queue block.
    """
    import math
    ...
    maneuver = data.get('maneuver', data.get('maneuverCode', ''))
    
    # Validation check: verify the session cookie matches the player executing the move
    ...
    try:
        # Hand off vehicle calculation mechanics to the GameEngine class module
        success, message = GameEngine.process_player_movement(game_id, username, maneuver)
        
        if not success:
            return jsonify({"error": message}), 400
            
        # Gather active match tracking files ordered sequentially to target the latest file
        game_dir = game_dir_path(game_id)
        all_files = os.listdir(game_dir) if os.path.isdir(game_dir) else []
        ...
        # Prune past unconfirmed preview steps by this user to refresh ghost overlays cleanly
        file_data = [
        ...
        # Find the active tracking record for the player to mirror basic styling metrics
        base_car = next(
            (r for r in file_data if str(r.get('CarPosition', '')).replace(' ', '') == 'CarPosition' and r.get('owner') == username),
            None
        )
        
        has_proposed = any(str(r.get('ProposedCarPosition', '')).replace(' ', '') == 'ProposedCarPosition' and r.get('owner') == username for r in file_data)
        
        if not has_proposed and base_car:
            ...
            elif maneuver.startswith('D') and len(maneuver) >= 3 and maneuver[-1] in ['L', 'R']:
                severity = int(maneuver[1:-1])
            ...
            step_cost = 0.5 if maneuver == 'half' else 1.0
            calc_rem = round(max(0.0, float(base_car.get('remaining', 2.0)) - step_cost), 1)
            calc_full = max(0, int(base_car.get('full_remaining', 2)) - (0 if maneuver == 'half' else 1))
            calc_half = 0.0 if maneuver == 'half' else float(base_car.get('half_remaining', 0.0))
            ...
            fallback_ghost = {
                ...
                'segment_index': 1,
                'total_segments': 2 if (int(base_car.get('current_speed', 0)) == 60) else 1,
                'remaining': calc_rem,
                'full_remaining': calc_full,
                'half_remaining': calc_half,
                'maneuvered': True if maneuver != 'STR' else bool(base_car.get('maneuvered', False)),
                'timestamp': datetime.now().isoformat()
            }
            file_data.append(fallback_ghost)
            
        # --- FIX: THE PREVIEW RE-WRITE HOOK IS REMOVED HERE ---
        # We no longer search for the MovementQueue or subtract values in this route.
        # The main resource file remains 100% untouched until Confirm is pressed.
        # ──────────────────────────────────────────────────────
        
        write_game_file(filepath, file_data)
        return jsonify({"status": "success", "message": "Maneuver preview staged successfully."}), 200
        
    except Exception as e:
        return jsonify({"error": f"Internal engine calculation crash during route handling: {str(e)}"}), 500
...
def handle_game_confirm_move():
    ...
    Enforces clean integer targets for car headings and orientations.
    """
    ...
    #    return jsonify({'error': f'Failed reading game directory: {str(io_err)}'}), 500
    ...
    try:
        ...
        # FIX 1: Extract real active maneuver before engine script index translation modifications
        ...
        active_pool = [p for p in players if not p['done']]
        if all(p.get('moved_this_segment', False) for p in active_pool):
            for p in players:
                p['moved_this_segment'] = False
                
        player = next((p for p in players if p['username'] == username and not p['done'] and not p['moved_this_segment']), None)
        ...
        if is_bend and player['maneuvered']:
            return jsonify({'error': 'Maneuver already exhausted this phase. Must travel straight.'}), 400
            
        if is_half:
            if player['half_remaining'] <= 0:
                return jsonify({'error': 'No half movement remaining.'}), 400
            player['half_remaining'] = 0.0
            player['remaining'] = round(max(0.0, player['remaining'] - 0.5), 1)
        else:
            if player['full_remaining'] <= 0:
                return jsonify({'error': 'No full movement remaining.'}), 400
            player['full_remaining'] = max(0, player['full_remaining'] - 1)
            player['remaining'] = round(max(0.0, player['remaining'] - 1.0), 1)
        ...
        move_record = {
            ...
            'username': username,
            'maneuver': maneuver, # Writes the real action (e.g. D1L) cleanly to disk log sheets
        ...
        file_data = read_game_file(filepath)
        # file_data.append(move_record) Let's not both to write this to the file
        ...
        return jsonify({
            'message': f'{username} moved: {maneuver}',
            'queue': queue_entry,
            'move_record': move_record,
            'phase_complete': queue_entry['complete']
        }), 200
...
def build_and_write_movement_queue2(game_id: str, game_name: str, phase: int) -> bool:
    """
    ...
    safe_name = sanitize_name_for_filename(game_name)
    phase_suffix = f'T{1}P{phase}M'  # always turn 1 for now
    filename = f'{game_id}{safe_name}{phase_suffix}.txt'
    filepath = os.path.join(GAME_FOLDER, filename)
    ...
    # Extract player speeds from CarPosition entries
    players = []
    for entry in data:
        # Strip structural space markers from keys to avoid parsing mismatch errors
    ...
    # Compile base structure through your updated game_tables function
    queue = build_movement_queue(players, phase)
    
    # Hardcode validation parameters immediately inside the file write sequence
    ...
    # Clean out any legacy tracking entries before committing payload to disk
...
def get_game_state2(game_id):
    """
    Locates the active match database file, enforces MapRenderer space-stripping 
...
# app.py (New Operational Combat Matrix Endpoint Route)
...
def calculate_live_to_hit():
    ...
    data = request.get_json()
    game_id = data.get('game_id')
    # PATCH: was `weapon_index = int(data.get('weapon_index', 1))` --
    # now the canonical weapon/link id string ("weapon-0-1", "link-2"),
    # matching get_firing_dialog_data / confirm_firing_action.
    ...
    mods = CombatModifiersEngine.evaluate_attack_modifiers(attacker, defender, weapon_mock, "Chassis", distance)
...
def get_firing_dialog_data():
    ...
    try:
        ...
        driver_gunner_qty = GameEngine.to_int(car_record.get('self.var_driver_gunner_qty', 0), default=0)
        crew_titles = GameEngine.get_crew_titles(driver_gunner_qty)
        used_crew = car_record.get('firing_actions_used_this_turn', [])

        crew_list = [
            {"title": title, "available": title not in used_crew}
...
def confirm_firing_action():
    """
    Marks a crew member and a weapon/accessory as having used their
    firing action for this turn. Does not resolve to-hit or damage —
    that's handled separately. This just records the action as spent.
    """
    data = request.get_json() or {}
    game_id = str(data.get('game_id', '')).strip()
    crew_title = str(data.get('crew_title', '')).strip()
    weapon_id = str(data.get('weapon_id', '')).strip()
    username = session['username']

    if not game_id or not crew_title or not weapon_id:
        return jsonify({'error': 'game_id, crew_title, and weapon_id are required'}), 400
    ...
    try:
        ...
        # 2. Validate the crew member exists and hasn't already fired this turn
        driver_gunner_qty = GameEngine.to_int(car_record.get('self.var_driver_gunner_qty', 0), default=0)
        valid_crew_titles = GameEngine.get_crew_titles(driver_gunner_qty)

        if crew_title not in valid_crew_titles:
            return jsonify({'error': f'"{crew_title}" is not a valid crew member for this vehicle'}), 400

        used_crew = car_record.get('firing_actions_used_this_turn', [])
        if crew_title in used_crew:
            return jsonify({'error': f'{crew_title} has already taken a firing action this turn'}), 400

        # 3. Validate the weapon/accessory/link exists and hasn't already been used this turn
        link_actions = GameEngine.get_link_actions(car_record)
        single_actions = GameEngine.get_available_firing_actions(car_record)
        ...
        # 4. Record the action(s) as spent and write the snapshot back
        used_crew.append(crew_title)
        ...
        return jsonify({
            'message': f'{crew_title} fired using {weapon_id}',
        ...
        return jsonify({'error': 'Failed to confirm firing action'}), 500
# ── Entry Point ───────────────────────────────────────────────────────────────
